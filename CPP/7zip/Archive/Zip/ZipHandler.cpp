        ...
        Z7_COM7F_IMF(CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT* value))
        {
            ...
            */
            ...
            case kpidMethod:
            {
                ...
                if (id == NFileHeader::NCompressionMethod::kCtEncryption)
                {
                    CCtEnhancedZipExtra ctField;
                    if (extra.GetCtEnhancedZip(ctField)) {
                        m += ctField.Props.GetCipherNameAndBits();
                        m.Add_Space_if_NotEmpty();
                        m.Add_Slash();
                        m.Add_Space_if_NotEmpty();
                        m += ctField.Props.GetMacName();
                        id = ctField.Props.Method;
                        isCtEncryption = true;
                    }
                }

                if (item.IsEncrypted())
                    if (!isWzAes && !isCtEncryption)
                    {
...
public:
    ...
    class CZipDecoder
    {
        ...
        CMyComPtr2<ICompressFilter, NCrypto::NCtCipherCoder::CDecoder> _ctCipherDecoder;
    ...
    Int32& res)
{
    ...
    if (id == NFileHeader::NCompressionMethod::kCtEncryption) {
        if (item.GetMainExtra().GetCtEnhancedZip(ctField)) {
            if (!item.IsEncrypted())
            {
                res = NExtract::NOperationResult::kUnsupportedMethod;
                return S_OK;
            ...
            ctEnhancedMode = true;
            needCRC = false; // CTEnhancedZip relies on HMAC, which is much stronger than CRC.
        }
    }

    if (!wzAesMode && !ctEnhancedMode)
        if (item.IsEncrypted())
        {
            if (item.IsStrongEncrypted())
            {
...

    {
        ...
        else if (ctEnhancedMode)
        {
            const UInt64 overhead = ctField.Props.GetMacSize();
            if (packSize < overhead)
                return S_OK;
            packSize -= overhead;
        }
    ...
    if (item.IsEncrypted())
    {
        ...
        {
            id = ctField.Props.Method;
            _ctCipherDecoder.Create_if_Empty();   // CREATE NEW ONE
            cryptoFilter = _ctCipherDecoder;
            if (!_ctCipherDecoder->SetProps(ctField.Props))
            {
                res = NExtract::NOperationResult::kUnsupportedMethod;
                return S_OK;
            }
...

    {
        ...
        if (item.IsEncrypted())
        {
            ...
            {
                result = _ctCipherDecoder->ReadHeader(inStream);
                if (result == S_OK)
                {
                    if (!_ctCipherDecoder->Init_and_CheckLegacy())
                    {
                        res = NExtract::NOperationResult::kWrongPassword;
                        return S_OK;
                    }
    ...
    else if (ctEnhancedMode)
    {
        ...
        {
            const unsigned macSize = (unsigned)ctField.Props.GetMacSize();
            inStream->Init(macSize);
            if (_ctCipherDecoder->CheckMac(inStream, authOk) != S_OK)
                authOk = false;
        }
    }
