...
/* ========================================
 * ADD THESE FUNCTIONS TO THE END OF Camellia.c
 * After Camellia_DecryptBlock (line 1461)
 * ========================================
 */

 /* Camellia CTR mode encryption/decryption
  * Analogous to AesCtr_Code from Aes.c
  *
  * p points to: [counter (4 x u32)] [keyTable (68 x u32)]
  * The counter is stored in little-endian format (like AES CTR)
...
{
    KEY_TABLE_TYPE* keyTable = (KEY_TABLE_TYPE*)(p + 4);

    for (; numBlocks != 0; numBlocks--)
    {
        unsigned char counterBlock[CAMELLIA_BLOCK_SIZE];
        unsigned char encryptedCounter[CAMELLIA_BLOCK_SIZE];
        unsigned i;

        /* Increment 128-bit counter (big-endian interpretation: p[3] is LSW, p[0] is MSW) */
        if (++p[0] == 0)
            if (++p[1] == 0)
                if (++p[2] == 0)
                    ++p[3];

        /* Convert current counter value to big-endian byte array
         * Camellia uses big-endian byte order internally */
        memcpy(counterBlock, p, CAMELLIA_BLOCK_SIZE);
        ...
        /* XOR encrypted counter with data */
        for (i = 0; i < CAMELLIA_BLOCK_SIZE; i++)
        {
            data[i] ^= encryptedCounter[i];
        }
    ...
    int keyBitLength, const unsigned char* iv)
{
     ...
     * Convert big-endian bytes to u32 values for counter storage */
    for (i = 0; i < 4; i++)
    {
        p[i] = GETU32(iv + i * 4);
    }
    ...
    Camellia_Ekeygen(keyBitLength, key, *keyTable);
}
